...
class Turtlebot:
    ...
    def __init__(self, current_time=None):
        ...
        self.x = 0.0
        self.y = 0.0
        # flag value will be used for switching between rotation and translation
        self.flag = False
        self.theta = 0.0
        self.roll = 0.0
        self.pitch = 0.0
        ...
        # this will be an input to actuator response
        ...
        # This will help us in controlling the actuator response
        self.feedback_value = 0.0
        # We will compare our feedback_value to the SetPoint value
        self.SetPoint = 100
        # We will use time difference of short intervals to calculate Integral and Derivative Terms
        self.current_time = current_time if current_time is not None else time.time()
        # After each iteration we will update the last_time and current time
        ...
        # After each iteration we will update the last_error.
        self.last_error = 0.0
        self.error = 1000.0
    ...
    def get_odom(self, request):
        ...
        # We will only use the yaw value, or the angle (theta) with z axis.
        (self.roll, self.pitch, self.theta) = euler_from_quaternion(
            [self.turtle.x, self.turtle.y, self.turtle.z, self.turtle.w])
        print("in callback 2")
    ...
    # Calculates the angle by which the turtlebot has to turn
    def turn_angle(self, new_y, new_x):
        return (atan2(new_y, new_x))

    # Calculates Angular Velocity
    def calculate(self, new_y, new_x, Kp, Ki, Kd, feedback_value, current_time=None):
        ...
        # Output is the updated angular or linear velocity
            ...
            while(((self.dist(self.new_y, self.new_x)) > 0.1) and (i < self.n_points)):
                ...
                if((self.flag == False) and (round(abs(self.SetPoint-(self.theta)), 3) >= 0.005)):
                    ...
                    if (i == 0):
                        ...
                        self.SetPoint = atan2(
                            self.goal.y - self.n.y[i-1], self.goal.x - self.n.x[i-1])
                    print(self.SetPoint)
                    ...
                    vel_msg.angular.z = self.cal_easy(
                        self.new_y, self.new_x, self.Kp, self.Ki, self.Kd, self.feedback_value)     # Simple P Controller
                else:
                    if(self.flag == False):
                ...
                if ((self.flag == True) and (self.dist(self.new_y, self.new_x) > (0.1 + i/20))):   # Precision is 0.1
                    ...
                    if (i == 0):
                        ...
                        self.SetPoint = self.dist(
                            self.goal.y - self.n.y[i-1], self.goal.x - self.n.x[i-1])
                        self.feedback_value = self.dist(
                            self.y - self.n.y[i-1], self.x - self.n.x[i-1])
                    print("--------")
                    print(self.SetPoint)
                    print(self.goal)
                    ...
                    vel_msg.linear.x = self.cal_easy(
                        self.new_y, self.new_x, self.Kp, self.Ki, self.Kd, self.feedback_value)        # Simple P Controller
                    print("===========")
